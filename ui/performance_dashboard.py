...
class PerformanceDashboard:
    ...
    def __init__(self, parent):
        ...
        ttk.Button(footer_frame, text="Close", command=self.close).pack(side=tk.RIGHT, padx=10)
    ...
    def _setup_overview_tab(self) -> None:
        ...
        # Create a treeview for metrics
        self.metrics_tree = ttk.Treeview(metrics_frame, columns=("value",), show="headings")
        ...
        # Create a treeview for batch metrics
        self.batch_tree = ttk.Treeview(batch_frame, columns=("value",), show="headings")
    ...
    def _setup_latency_tab(self) -> None:
        ...
        self.latency_tree = ttk.Treeview(frame, columns=("avg", "p50", "p95", "p99", "max"), show="headings")
    ...
    def _update_overview_tab(self) -> None:
        ...
        total_connections = self.performance_data.get("total_connections", 0)
        self.connections_label.config(text=f"{active_connections}/{total_connections}")
        
        messages = self.performance_data.get("total_messages", 0)
        ...
        # Update performance metrics tree
        ...
        metrics = [
            ...
            ("WebSocket Processing", self.performance_data.get("websocket_message_processing", 0)),
        ]
        
        for name, value in metrics:
            self.metrics_tree.insert("", "end", values=(name, f"{value:.2f}"))
            
        # Update batch metrics tree
    ...
    def _update_latency_tab(self) -> None:
        ...
        for component in components:
            ...
            avg = self.performance_data.get(f"{key}_avg", 0)
            p50 = self.performance_data.get(f"{key}_p50", 0)
            p95 = self.performance_data.get(f"{key}_p95", 0)
            p99 = self.performance_data.get(f"{key}_p99", 0)
            max_val = self.performance_data.get(f"{key}_max", 0)
            
            self.latency_tree.insert("", "end", text=component, values=(
                f"{avg:.2f}", f"{p50:.2f}", f"{p95:.2f}", f"{p99:.2f}", f"{max_val:.2f}"
            ))
    ...
    def _schedule_refresh(self) -> None:
        ...
        if self.auto_refresh and hasattr(self, 'window'):
            self.refresh()
            self.window.after(self.refresh_interval, self._schedule_refresh)
